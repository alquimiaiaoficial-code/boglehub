...
const FAQ = [
  ...
  { q: 'What is the two-month rule (regla dos meses)?', a: 'Wash sale rule in Spain: if you sell an ETF or stock at a loss and rebuy the same security within 2 months before or after, the loss is disallowed for tax purposes. To realize a tax loss, wait more than 2 months before rebuying the same ticker, or buy a different ETF with similar exposure. The 2-month window applies to securities admitted to trading. Non-listed securities fall under a 1-year window, and whether index fund units belong in one category or the other is unsettled, so 12 months is the only period compatible with both readings.' },
]
...
export default function TaxationEn() {
  return (
    <>
      ...
      <DescargoFiscal variant="fiscal" lang="en" />
      <Footer />
    </>
  )
}
