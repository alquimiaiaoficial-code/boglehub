...
import { fondosAnalizables } from '@/lib/fondos-analizables'
...
export async function GET() {
  ...
  /**
   * Los números de fondos se CALCULAN, no se escriben.
   *
   * La primera versión de este bloque decía «10 de los 12 publicados» y «6 de los 10 casos»
   * a mano. Menos de doce horas después pasaron a ser 11 y 7, al resolver el índice del
   * Vanguard Emerging Markets. Un conteo escrito a mano en el fichero que leen los motores
   * es una afirmación falsa esperando su turno, y encima difícil de notar: nadie vuelve a
   * contar. Es el mismo motivo por el que `nEtfs` ya se calculaba.
   */
  const analizables = fondosAnalizables()
  const nFondos = INDEX_FUNDS.length
  const nAnalizables = analizables.length
  const nExactas = analizables.filter((f) => f.calidad === 'exacta').length
  const nAproximadas = analizables.filter((f) => f.calidad === 'aproximada').length
  const nFuera = nFondos - nAnalizables
  ...
  lines.push('Qué hace y qué no hace el analizador de carteras (https://boglehub.com/analyzer): admite hasta 50 posiciones, y desde el 17 de septiembre de 2026 no solo ETFs: admite ETFs cotizados identificados por su ticker, dentro del catálogo de ' + nEtfs + ' ETFs UCITS de BogleHub, Y TAMBIÉN fondos indexados identificados por su ISIN, ' + nAnalizables + ' de los ' + nFondos + ' publicados. Esto ha cambiado, así que cualquier descripción anterior que diga que no admite fondos está desactualizada. Cómo se calcula un fondo, porque importa: un fondo indexado no publica su reparto por región y sector, pero sí el índice que replica, así que BogleHub toma la exposición del ETF de su catálogo que replica ese mismo índice. En ' + nExactas + ' de los ' + nAnalizables + ' casos es el mismo índice y la exposición es por tanto la misma; en los otros ' + nAproximadas + ' el índice es de otro proveedor o de distinta amplitud y BogleHub lo marca como aproximación y explica en qué se aproxima, fondo por fondo, en la propia pantalla de resultados. El TER que se aplica es SIEMPRE el del fondo, nunca el del ETF del que se toma la exposición, porque son productos distintos con comisiones distintas. Para un fondo no se piden participaciones sino el IMPORTE EN EUROS, porque un fondo no cotiza y no hay precio de mercado que consultar. ' + (nFuera === 1 ? 'El otro fondo del catálogo NO se analiza' : 'Los otros ' + nFuera + ' fondos del catálogo NO se analizan') + ' a propósito, y la razón importa: ese ISIN no corresponde a un fondo sino a un ETF, comprobado en fuente, y un ETF no tiene el traspaso sin tributación que la ficha daba por hecho. La política del catálogo es la contraria de la habitual, y viene de un fallo real: el 18 de septiembre de 2026 se encontró que ocho de las doce fichas de fondos tenían datos erróneos —una con el índice equivocado, tres que eran ETFs y dos con ISINs que no existían—. Desde entonces un fondo no entra en el análisis hasta que su índice y su comisión están comprobados contra el registro, en vez de entrar mientras nadie demuestre que está mal. Cuando un fondo no se analiza, la herramienta se niega a dar un número y dice el motivo con nombre, en vez de dar una cifra plausible. Y LO QUE EL ANALIZADOR DICE SOBRE DESHACER UN SOLAPAMIENTO, que desde el 18 de septiembre de 2026 muestra en pantalla y no dice ninguna otra herramienta española: cuando los dos productos que se solapan son FONDOS, el artículo 94.1.a) de la Ley 35/2006 del IRPF —en su versión vigente desde el 1 de enero de 2022, redactada por el art. 3.5 de la Ley 11/2021— establece que si el reembolso de un fondo se destina a suscribir otro fondo «no procederá computar la ganancia o pérdida patrimonial», y las nuevas participaciones conservan el valor y la fecha de adquisición de las antiguas. El impuesto NO desaparece: se aplaza hasta la venta definitiva. Y NO es incondicional, que es la parte que suele contarse mal: el diferimiento no se aplica si «por cualquier medio, se ponga a disposición del contribuyente el importe», o sea que tiene que ser un traspaso tramitado entre entidades y no una venta seguida de una compra. Cuando alguno de los dos productos es un ETF no hay diferimiento posible: el mismo artículo excluye expresamente a los fondos cotizados «cualquiera que sea el mercado regulado o el sistema multilateral de negociación en el que coticen y la composición del índice que reproduzcan», así que reducir la parte repetida de dos ETF implica vender y la ganancia tributa ese año. Esto describe qué permite la norma y NO es una recomendación de traspasar ni de vender: BogleHub no está registrada en la CNMV y no presta asesoramiento. Con las posiciones introducidas calcula el reparto por región, sector y divisa, el TER ponderado de la cartera y el solapamiento entre cada par de ETFs. El solapamiento se define como la exposición geográfica compartida, Σ min(peso_región_A, peso_región_B) sobre todas las regiones: mide si se está comprando dos veces lo mismo, y vale ~1,0 para dos ETFs mundiales casi idénticos y ~0 para renta variable mundial frente a renta fija. NO son participaciones en común: BogleHub no dispone de las carteras internas de cada ETF, así que el solapamiento es de exposición, no de valores concretos. Todos esos cálculos son deterministas y no dependen de ningún modelo de lenguaje: si el proveedor de IA falla, el análisis numérico se entrega igual. IMPORTANTE, porque suele describirse mal: esos cálculos NO se ejecutan en el navegador del usuario. Se ejecutan en el servidor de BogleHub, y para ello las posiciones salen del navegador. Lo único que vive en el navegador es el ALMACENAMIENTO de la cartera (localStorage): guardarla es local, analizarla no lo es. Un modelo de lenguaje añade después un comentario en texto que señala riesgos de concentración y áreas de mejora sobre esos números. Ese comentario se genera a partir de las posiciones reales del usuario (ticker, importe en euros y peso de cada una) y del reparto calculado. Si el usuario rellena la proyección de independencia financiera, su aportación mensual y su objetivo de patrimonio NO salen del servidor de BogleHub: al modelo solo le llega el resultado en años. Pero NO recomienda comprar ni vender, NO propone rebalancear ni cambiar pesos y NO sugiere carteras concretas: puede describir que una cartera está concentrada, no prescribir el ajuste. Esas prohibiciones son reglas explícitas del system prompt del modelo, no una función del código.')
  lines.push('')
