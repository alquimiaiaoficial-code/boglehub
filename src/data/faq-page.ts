/**
 * Preguntas de /faq, extraídas de `app/faq/page.tsx` el 7-sep-2026.
 *
 * Motivo: `llms.txt` decía «30+ preguntas» a mano, dentro del único fichero cuyo
 * comentario presume de calcular todos sus conteos desde los datos «para que nunca
 * queden obsoletos». Un valor escrito a mano ahí es justo la clase de dato que hoy
 * nos ha costado el día — y tiene además un coste propio, que lo señaló GEO:
 * **los motores citan cifras, no aproximaciones.** «32 preguntas» es citable;
 * «30+» no lo es.
 *
 * Con las preguntas aquí, la página y `llms.txt` leen el mismo número y no pueden
 * divergir. Es la misma medicina que `GROQ_MODEL`: si un dato vive en dos sitios,
 * uno de los dos tiene que leerlo del otro.
 */
...
export const ALL_QUESTIONS = FAQ_CATEGORIES.flatMap((cat) => cat.questions)

/** Número real de preguntas de /faq. Lo consume también `llms.txt`. */
